...
survey <- read.xlsx("~/Fieldwork SPH/Data/PH750-2 online tech 07-09.xlsx", sheetIndex=1, header=TRUE, colIndex=cols,
                    colClasses=c("character", "character", rep("numeric",4),"character","character",rep("numeric",4),rep("character",5),"numeric",
                                 "character","character","numeric","numeric",rep("character",4),"numeric", "character","character","numeric",
                                 rep("character",6),"numeric","numeric") )
...
colnames(survey) <- colnamesfile$V2[compl==TRUE]
names(survey)
head(survey,10)
